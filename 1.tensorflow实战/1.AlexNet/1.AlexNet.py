...
def print_actications(t):
    ...
    Args:
        t: tensor
    Function: 展示每一个卷积层或池化层输出tensor的尺寸
    Returns: None
    '''
...
def inference(images):
    ...
    Args:
        images: 输入的图片
    Funcion: 实现神经网络的前5层卷积操作
    Returns: 
        pool5: 5层卷积操作的输出tensor
        parameters: 列表，其中包含5层卷积操作中的参数，每一层都有kernel和bias
    '''
    parameters = []
...
def time_tensorflow_run(sess, target, info_string):
    """
    Args:
        sess: tensorflow中的 Session
        target: 需要评测运算的算子
        info_string: 测试的名称
    Function: 评估AlexNet每轮计算时间
    Returns: pass
    """
    num_step_burn_in = 10 
    total_duration = 0.0
...
# 实现 三层全连接网络
# AlexNet 网路中 还需要添加 3层全连接层，隐藏结点的数目为: 4096、4096、1000 


def run_benchmark():
    '''
    Args: None
    Function: 主函数，主要运行整个模型
    Returns: None
    '''
